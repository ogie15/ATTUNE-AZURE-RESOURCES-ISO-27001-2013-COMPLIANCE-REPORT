...
foreach ($item in $AllAZPolicies) {
    ...
    $ComplianceStates = $AllComplianceStates | Where-Object { $_.PolicyDefinitionId -like ($item).PolicyDefinitionId -and $_.ComplianceState -like "NonCompliant" -and $_.PolicyAssignmentScope -like "/subscriptions/8d82306b-37fe-4d67-bb74-ebf28b32517b"}
    ...
    foreach ($ComplianceStateloop in $ComplianceStates) {
        ...
        if (
            ...
            -or $PolicyDisplayName -match 'API App should only be accessible over HTTPS'`
            ...
            -or $PolicyDisplayName -match 'Function App should only be accessible over HTTPS'`
            ...
            -or $PolicyDisplayName -match 'Auditing on SQL server should be enabled'`
